...
def readGCCO(hdul):
    ...
    # GCCO data has 22 orders
    n_orders, s, w = 22, [], []

    for i in range(0,1):
        print('{}: {}'.format(i, hdul[2].data[i]))
    
    # Read the data from the fits file
    for ordind in np.arange(n_orders):
        s.append(np.array(hdul[2].data[ordind]))
        w.append(np.array(hdul[6].data[ordind]))

    # Read the barycentric correction
    bcv = 10 # hdul[0].header["RVCC"] - hdul[0].header["RCC"] * 299792.458

    # Read the time of observations
    t   = 20000 # hdul[0].header["BJD"]
...
obs_list = Observations(read_function=readGCCO, target_visits_lib='/home/darin/Spectral-analysis/spectra_files/Vega')
...
print('Rotating.', end=' ')
template.RotationalBroadening(vsini=20.48, epsilon=0.15)
